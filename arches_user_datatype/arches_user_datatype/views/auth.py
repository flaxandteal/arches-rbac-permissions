...
from arches_user_datatype.utils.forms import PersonUserCreationForm
from arches_user_datatype.models import RegistrationLink
...
def _person_decrypt(person):
    ...
    Person = ResourceTileTree.get_tiles(graph_slug=settings.SIGNUP_NODEGROUP["graph_slug"], as_representation=True)
    logger.debug("Decrypting for person %s", str(person_id))
    person = Person.get(resourceinstanceid=person_id)
...
class PersonSignupView(View):
    def get(self, request):
        form = PersonUserCreationForm(enable_captcha=False)
    ...
    def post(self, request):
        ...
        form = PersonUserCreationForm(postdata, enable_captcha=settings.ENABLE_CAPTCHA)
        ...
        if form.is_valid():
            ...
            confirmation_link = request.build_absolute_uri(reverse("person_confirm_signup") + "?" + url_encrypted_userinfo)
...
class PersonConfirmSignupView(View):
    def get(self, request):
        ...
        form = PersonUserCreationForm(userinfo)

        if (ng := person.aliased_data.user_account) and (nd := ng.aliased_data.user_account) and isinstance(nd["node_value"], dict) and nd["node_value"].get("userId"): # Ensure we are not seeing incorrect "null-value" data
            # TODO: translation
            ...
            if datetime.fromtimestamp(userinfo["ts"]) + timedelta(days=1) >= datetime.fromtimestamp(int(time.time())):
                if form.is_valid():
                    ...
                    tile.nodegroup_id = settings.SIGNUP_NODEGROUP["nodegroup_id"]
                    tile.data = {
                        settings.SIGNUP_NODEGROUP["node_id"]: {
                            "userId": int(user.pk)
                        }
                    }
                    tile.save()

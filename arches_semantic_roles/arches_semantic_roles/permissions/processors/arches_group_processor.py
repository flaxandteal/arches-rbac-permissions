...
# TODO: removing for now to keep it simple, could be an add on.
...
class ArchesGroupProcessor:
    def __init__(self, enforcer, operator):
       ...
       self.permission_processor = GroupPermissionProcessor(enforcer, operator)
    ...
    def _process_group_members(self, group: Group, group_key: str, ancestors: List[Group]) -> None:
        ...
        for n, member in enumerate(group.members):
            if isinstance(member, Group):
                member_key = self._operator._subj_to_str(member)
    ...
    def _sync_django_groups(self, group: Group, group_key: str, users: List[User]) -> None:
        ...
        if len(group.django_group) == 0:
            self._operator._ri_to_django_groups(group)

...
from arches_semantic_roles.utils.sets import UnindexedError, get_sets_for_resource, get_index
from arches_semantic_roles.service import trigger as trigger # TODO: remove other refs
...
class CasbinPermissionFramework(ArchesPermissionBase):
    ...
    @property
    def _enforcer(self):
        return self._casbin_db._enforcer

    @property
    def _casbin_db(self):
        # This is a new issue,
        # since this gets imported earlier than in Coral, we need
        # to ringfence the database-defined classes
        from ._casbin_db import CasbinDB
        return CasbinDB(GRAPH_REMAPPINGS)
    ...
    def _subj_to_str(self, subj):
        return self._casbin_db._subj_to_str(subj)

    def _obj_to_str(self, obj):
        return self._casbin_db._obj_to_str(obj)

    def recalculate_table(self):
        from .casbin_policy_builder import CasbinPolicyBuilder
        policy_builder = CasbinPolicyBuilder(self._enforcer, self._casbin_db)
        return policy_builder.rebuild_policies()
    ...
    def update_groups_for_user(self, user):
        if not isinstance(user, User):
            return # TODO: This gets triggered, seemingly unintentionally, with a group
    ...
    def assign_perm(self, perm, user_or_group, obj=None):
        ...
        if isinstance(user_or_group, DjangoGroup):
            group = user_or_group
        elif isinstance(user_or_group, self._casbin_db.Group):
            group = self._casbin_db._ri_to_django_group(user_or_group)
    ...
    # Remove these for Application for first go
    ...
    def get_allowed_instances(
        ...
        subj = self._subj_to_str(user)
        graphs = self._enforcer.get_implicit_permissions_for_user(subj)
        perms = self.get_default_settable_permissions()
        for _, graph, act in graphs:
            if not graph.startswith("r:"):
                continue
            if act in perms or act == "__all__":
                allowed.add(graph[3:])
        return list(allowed)

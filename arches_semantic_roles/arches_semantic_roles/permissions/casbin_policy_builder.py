...
class CasbinPolicyBuilder:
    ...
    def __init__(self, enforcer, operator):
        self._enforcer = enforcer
        self._operator = operator
        self.sets = []
        self.django_processor = DjangoGroupProcessor(enforcer, operator)
        self.arches_group_processor = ArchesGroupProcessor(enforcer, operator)
        self.set_processor = SetProcessor(enforcer, operator)
    ...
    def _process_django_user_groups(self):
        ...
        for user in User.objects.all():
            user_key = self._operator._subj_to_str(user)
            for group in user.groups.all():
                group_key = self._operator._subj_to_str(group)
    ...
    def _trigger_reload_if_needed(self):
        ...
        from arches_semantic_roles.service import trigger

...
class PersonUserSignupView(BaseManagerView):
    ...
    def post(self, request):
        ...
        # Should we make it dynamic? If there is an ontology type
        # for Person, why can't we have different models of a Person-like
        # type able to log in?
        person_graph = Graph.objects.get(name="Person")
        # TODO: tidy up to do a single query, we don't need graph
        person = Resource.objects.get(
            graph_id=person_graph.graphid,
        ...
        # TODO: from ORM, but Django would error before now
        # if not person:
        ...
        link.save()
        encrypted_person_id = AES.encrypt(f"{person.resourceinstanceid}:{link.registrationlinkid}")
        url_encrypted_data = urlencode({"person": encrypted_person_id})
        user_signup_link = request.build_absolute_uri(reverse("person_signup") + "?" + url_encrypted_data)

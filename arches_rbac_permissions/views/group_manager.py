...
MAX_DEPTH = 5 # TODO: make settable
...
class GroupManagerView(View):
    ...
    def get(
        ...
        one_graph = bool(request.GET.get("oneGraph", False))
        # Less brittle to control this with an ontology, but we cannot rely on
        # one being present.
        ...
        def _test_relationship(resourceid, rr):
            ...
            return (
                back and str(resourceid) == str(rr["from_resource"]) and (not one_graph or str(graph.graphid) == str(rr["from_resource_graph"]))
            ) or (
                forward and str(resourceid) == str(rr["to_resource"]) and (not one_graph or str(graph.graphid) == str(rr["to_resource_graph"]))
            )
            ...
            for resourceid in new_resources:
                ...
                if related_resources.get("resource_relationships"):
                    ...
                    relevant_resources = set(sum([
                        ...
                        if (not one_graph or str(rr["to_resource_graph"]) == str(graph.graphid))
                    ], []))
                    all_resources["related_resources"].update({
                        r["resourceinstanceid"]: r
                        for r in related_resources["related_resources"]
                        if r["resourceinstanceid"] in relevant_resources
                    })

...
import 'views/components/plugins/group-manager-graph';
...
const viewModel = Backbone.View.extend({
    initialize: function(options) {
        ...
        this.getTree = function() {
            $.ajax({
                type: "GET",
                url: self.urls.group_manager(self.activeGrouping()),
                data: {
                    oneGraph: true
                },
                context: self
        ...
        this.createResource = function(resourceinstanceid) {
            ...
            return {
                ...
                get: function(newPage) {
                    // TODO: check if anything needs fixed for pagination
                    var page = newPage || 1;
                    $.ajax({
                        url: self.urls.group_manager_for_resource(self.activeGrouping(), resourceinstanceid),
                        context: this,
                        dataType: 'json',
                        data: {
                            page: page,
                ...
                save: function(candidateIds, relationshipProperties, relationshipIds) {
                    ...
                    $.ajax({
                        url: self.urls.related_resources(resourceinstanceid),
                        data: payload,
                        context: this,
                        type: 'POST',
                        dataType: 'json'
                ...
                delete: function(relationshipIds) {
                    ...
                    $.ajax({
                        url: self.urls.related_resources(resourceinstanceid) + '?' + $.param(payload),
                        type: 'DELETE',
                        context: this,
                        dataType: 'json'
                    })
        ...
        var url = ko.observable(self.urls.search_results);
        this.url = url;
        this.select2Config = {
            ...
            onSelect: function(item) {
                $.ajax(self.urls.related_resource_candidates, {
                    dataType: 'json',
    ...
    updateTile: function(options, relationship) {
        var self = this;
        self.loading(true);
        window.fetch(self.urls.api_tiles(relationship.tileid()), {
            method: 'GET',
            credentials: 'include',
            headers: {
